        ...
        <CardContent>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            {locations.map((loc) => (
              <div
                key={loc.name}
                className={`group relative overflow-hidden rounded-lg shadow-md ${loc.name !== 'Canteen' ? 'cursor-pointer' : 'cursor-default'}`}
              ...
              >
                <Image
                  ...
                  width={400}
                  height={300}
                  className={`w-full h-56 object-cover transition-transform duration-300 ease-in-out ${loc.name !== 'Canteen' ? 'group-hover:scale-110' : ''}`}
                  data-ai-hint={loc.hint}
                />
                <div className={`absolute inset-0 bg-gradient-to-t from-black/60 to-transparent ${loc.name !== 'Canteen' ? 'group-hover:bg-black/40' : ''} transition-colors`} />
